...
func MessagePrefixFromString(prefix string) MessagePrefix {
	if len(prefix) == 0 || len(prefix) > 4 {
		logger.Errorf("Invalid prefix %s", prefix)
	}
	p := MessagePrefix{}
...
func RegisterMessage(prefix MessagePrefix, msg interface{}) {
	...
	if exists {
		logger.Errorf("Attempted to register message prefix %s twice",
			string(id[:]))
	}
	_, exists = MessageIDMap[t]
	if exists {
		logger.Errorf("Attempts to register message type %v twice", t)
	}
	MessageIDMap[t] = id
	MessageIDReverseMap[id] = t
}

// VerifyMessages calls logger.Error if message registration violates sanity checks
func VerifyMessages() {
	for t, k := range MessageIDMap {
		// No empty prefixes allowed
		if k[0] == 0x00 {
			logger.Error("No empty message prefixes allowed")
		}
		...
		for _, b := range k {
			if b == 0x00 {
				...
				logger.Error("No non-null bytes allowed after a nul byte")
			}
		}
		...
		for _, b := range k {
			if !((b >= '0' && b <= '9') || (b >= 'A' && b <= 'Z') ||
				(b >= 'a' && b <= 'z') || b == 0x00) {
				logger.Errorf("Invalid prefix byte %v", b)
			}
		}
		...
		if !mptr.Implements(reflect.TypeOf((*Message)(nil)).Elem()) {
			...
			logger.Errorf("Invalid message at id %d: %s", k, m)
		}
	}
	if len(MessageIDMap) != len(MessageIDReverseMap) {
		logger.Error("MessageIdMap mismatch")
	}
	// No empty prefixes
